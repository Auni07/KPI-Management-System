...
exports.getNotifications = async (req, res) => {
  try {
    ...
    const now = new Date();
    const soon = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000); // The due date is within the next 7 days
    let deadlineQuery = {};
    if (user.role === 'Staff') {
      deadlineQuery = { assignedTo: user._id, dueDate: { $gte: now, $lte: soon } };
    } else if (user.role === 'Manager') {
      // Manager can see all KPIs with upcoming deadlines
      deadlineQuery = { dueDate: { $gte: now, $lte: soon } };
    }
    ...
    // For Staff: New KPI assignments (With in the last 2 days)
    if (user.role === 'Staff') {
      ...
      kpisWithFeedback.forEach(kpi => {
        if (kpi.feedback && kpi.feedback.length > 0) {
          ...
          if (new Date(latest.date) > new Date(now.getTime() - 2 * 24 * 60 * 60 * 1000)) {
            notifications.push({
              type: 'comment',
              kpiId: kpi._id,
              kpiTitle: kpi.title
